...
import { shareInternal, createShareLink, revokeShareLink, updateShareLink, updateAccessPermission, revokeAccess, revokeAllFileAccess, markNotificationRead } from "./api";
...
export function useUpdateShareLink() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ linkId, data }: { linkId: string; data: Partial<CreateShareLinkInput> }) => updateShareLink(linkId, data),
        onSettled: () => {
            qc.invalidateQueries({ queryKey: shareKeys.all() });
        },
    });
...
export function useUpdatePermission() {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ accessId, permission }: { accessId: string; permission: string }) =>
