                ...
                <Card size="2" variant="surface" style={{ width: "380px", boxShadow: "var(--shadow-5)", display: "flex", flexDirection: "column", maxHeight: "400px" }}>
                    ...
                    >
                        ...
                        <Flex gap="2" onPointerDown={(e) => e.stopPropagation()}>
                            ...
                            <IconButton size="1" variant="ghost" color="gray" onClick={() => setIsMinimized(true)} title="Thu nhỏ thành bong bóng">
                                <X className="w-4 h-4" />
                            </IconButton>
                        </Flex>
                    </Flex>
                    ...
                    {/* Queue List */}
                    <Flex direction="column" gap="3" style={{ overflowY: "auto", flex: 1, paddingRight: "4px" }}>
                        {items.map((item, index) => {
                            ...
                            const canMoveUp = isPending && index > 0 && items[index - 1].status !== "UPLOADING";
                            const canMoveDown = isPending && index < items.length - 1;

                            return (
                                <Box key={item.id} p="2" style={{ backgroundColor: "var(--gray-a2)", borderRadius: "var(--radius-3)" }}>
                                    <Flex justify="between" align="start" gap="2" mb="2">
                                        <Box style={{ flex: 1, minWidth: 0 }}>
                                            <Text size="2" weight="medium" truncate style={{ display: "block" }}>
                                                {item.file.name}
                                            </Text>
                                        ...
                                        <Flex gap="1" align="center">
                                            ...
                                            {isUploading && (
                                                <IconButton size="1" variant="ghost" color="orange" onClick={() => pauseUpload(item.id)} title="Tạm dừng">
                                                    <Pause className="w-4 h-4" />
                                                </IconButton>
                                            )}
                                            {isError && (
                                                <IconButton size="1" variant="ghost" color="blue" onClick={() => retryUpload(item.id)} title="Thử lại">
                                                    <RefreshCw className="w-4 h-4" />
                                                </IconButton>
                                            )}
                                            {(isPending || isUploading || isPaused || isError) && !isCanceled && (
                                                <IconButton size="1" variant="ghost" color="red" onClick={() => cancelUpload(item.id)} title="Hủy">
                                    ...
                                    {(!isSuccess && !isError && !isCanceled) && (
                                        <Flex align="center" gap="2">
                                            <Box style={{ flex: 1 }}>
                                                <Progress value={item.progress} size="1" color={isPaused ? "amber" : "violet"} />
                                            </Box>
                                            {/* Priority reordering */}
                                            {isPending && (
                                                <Flex gap="1">
                                                    <IconButton size="1" variant="soft" color="gray" disabled={!canMoveUp} onClick={() => moveUp(item.id)}>
                                                        <ChevronUp className="w-3 h-3" />
                                                    </IconButton>
                                                    <IconButton size="1" variant="soft" color="gray" disabled={!canMoveDown} onClick={() => moveDown(item.id)}>
