...
export const OrderDetailsModal = () => {
    const { isDetailsModalOpen, selectedOrderId, closeDetailsModal, filters } = useOrdersStore();
    ...
    const handleUpdateStatus = (newStatus: string) => {
        if (!window.confirm(`Bạn có chắc muốn chuyển trạng thái đơn hàng này thành ${newStatus}?`)) return;
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                <div className="fixed inset-0 bg-gray-500/75 dark:bg-gray-900/90 transition-opacity backdrop-blur-sm" aria-hidden="true" onClick={closeDetailsModal}></div>
                    ...
                    <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div className="mt-2 space-y-4">
                            <div className="bg-gray-50 dark:bg-gray-900/50 p-4 rounded-lg flex justify-between items-center">
                                <div>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Mã Đơn</p>
                                    <p className="text-lg font-mono font-bold text-gray-900 dark:text-white mt-1">{order.orderCode}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Tình trạng</p>
                                    <p className={`text-sm font-bold mt-1 ${order.status === 'PENDING' ? 'text-yellow-600' :
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">Người dùng (ID)</p>
                                    <p className="text-sm font-medium text-gray-900 dark:text-white truncate" title={order.userId}>{order.userId}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">Gói đăng ký</p>
                                    <p className="text-sm font-medium text-blue-600 dark:text-blue-400">{order.planName}</p>
                                </div>
                                <div>
                            ...
                            <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                                <div className="flex justify-between">
                                    <span>Ngày tạo:</span>
                                    <span className="font-medium">{format(new Date(order.createdAt), "HH:mm:ss dd/MM/yyyy")}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Ngày hết hạn:</span>
                                    <span className="font-medium text-red-500">{format(new Date(order.expiredAt), "HH:mm:ss dd/MM/yyyy")}</span>
                                </div>
                                {order.confirmedAt && (
                                    <div className="flex justify-between">
                                        <span>Ngày duyệt:</span>
                                        <span className="font-medium text-green-600">{format(new Date(order.confirmedAt), "HH:mm:ss dd/MM/yyyy")}</span>
                                    </div>
                                )}
                            </div>
                        </div>
