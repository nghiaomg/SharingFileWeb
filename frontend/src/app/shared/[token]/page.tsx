...
import { Download, FileText, Image as ImageIcon, Video, Music, FileArchive, Loader2, Home, AlertCircle, Lock, Eye } from "lucide-react";
...
export default function SharedTokenPage() {
    ...
    const handlePreview = async () => {
        ...
        try {
            if (type === "folder") {
                const res = await fetch(`${API_BASE_URL}/public/share/${token}/folder${query}`);
    ...
    const fetchFile = async (pwd?: string) => {
        try {
            ...
            if (!res.ok) {
                if (data.msg === "REQUIRES_PASSWORD" || data.message === "REQUIRES_PASSWORD") {
    ...
    const handleDownload = async () => {
        ...
        try {
            const query = password ? `?password=${encodeURIComponent(password)}` : "";
            const res = await fetch(`${API_BASE_URL}/public/share/${token}/download${query}`);
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center">
                ...
                <p className="mt-4 text-muted-foreground font-medium">Đang tải tệp...</p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center p-4">
                <div className="w-full max-w-md bg-card border border-border rounded-2xl overflow-hidden shadow-sm p-10 text-center">
                    ...
                    <h2 className="text-xl font-bold text-foreground mb-2">Tệp được bảo vệ</h2>
                    <p className="text-muted-foreground text-sm mb-6">Nhập mật khẩu để truy cập tệp này.</p>
    ...
    if (error || !fileData) {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 text-center">
                ...
                <p className="text-muted-foreground mb-8 max-w-md">{error || "Link này không hợp lệ hoặc đã bị thu hồi."}</p>
                <Link href="/" className="flex items-center gap-2 px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary/90 transition-all">
    ...
    const canPreview = fileData.type === "folder" || fileData.type === "application/pdf" || fileData.type?.includes("spreadsheetml");
        ...
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-lg bg-card border border-border rounded-2xl overflow-hidden shadow-sm animate-in fade-in zoom-in-95 duration-500">
                <div className="p-10 flex flex-col items-center text-center">
                    ...
                    <h2 className="text-xl font-bold text-foreground mb-2 truncate w-full" title={fileData.name}>
                        {fileData.name}
                    </h2>
                    ...
                    <div className="w-full bg-muted/30 rounded-2xl p-5 mb-8 text-left border border-border/50 space-y-3">
                        ...
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-500 dark:text-muted-foreground">Quyền truy cập</span>
                        ...
                        {canDownload && fileData.type !== "folder" ? (
                            ...
                            <div className="w-full text-center p-4 bg-muted/50 border border-border/50 rounded-xl">
                                <p className="text-muted-foreground text-sm font-medium flex items-center justify-center gap-2">
                                    <Eye className="w-4 h-4" /> Link này chỉ cho phép xem, không cho tải xuống
                                </p>
                            </div>
                        )}
                    </div>
            ...
            {isPreviewOpen && (
                <PreviewModal
                    ...
                    itemType={fileData.type === "folder" ? "folder" : fileData.type === "application/pdf" ? "pdf" : fileData.type?.includes("spreadsheetml") ? "xlsx" : "unknown"}
                    fileUrl={previewUrl}
            ...
            {showRestrictedDialog && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setShowRestrictedDialog(false)} />
                    <div className="relative w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 fade-in duration-200">
                        <div className="p-6 text-center">
                            ...
                            <h3 className="text-xl font-bold text-foreground mb-2">Quyền truy cập hạn chế</h3>
                            <p className="text-muted-foreground text-sm">
                                Link này được thiết lập chỉ cho phép xem nội dung. Bạn không có quyền tải xuống tệp tin này.
                            </p>
                        </div>
            ...
            {showFeatureDevDialog && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-background/80 backdrop-blur-sm" onClick={() => setShowFeatureDevDialog(false)} />
                    <div className="relative w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 fade-in duration-200">
                        <div className="p-6 text-center">
                            <div className="w-16 h-16 bg-blue-500/10 rounded-full flex items-center justify-center mx-auto mb-4">
                                <AlertCircle className="w-8 h-8 text-blue-500" />
                            </div>
                            <h3 className="text-xl font-bold text-foreground mb-2">Tính năng đang phát triển</h3>
                            <p className="text-muted-foreground text-sm">
                                Tính năng tải từng tệp lẻ trong một thư mục chia sẻ theo Public Link hiện đang được chúng tôi phát triển. Vui lòng quay lại sau!
                            </p>
                        </div>
                        <div className="bg-muted/30 p-4 border-t border-border flex justify-center">
                            <button
