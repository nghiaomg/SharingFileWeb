      ...
      try {
        ...
        if (isAxiosError(err)) {
          if (err.response?.status === 404) {
            setError(
              "File này có thể không tồn tại hoặc đã bị tắt chia sẻ công khai.",
            );
          } else if (
            err.response?.status === 403 ||
            err.response?.data?.message?.includes("Forbidden") ||
            err.response?.data?.message?.includes("Unauthorized")
          ) {
            setError(
              err.response?.data?.message ||
                "Bạn không có quyền hoặc chưa đăng nhập để xem file này.",
            );
          } else {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await apiClient.get(
        `/files/public/download/${fileData.id}`,
        {
          responseType: "blob",
        },
      );
      ...
      if (isAxiosError(err)) {
        alert(
          err.response?.data?.message ||
            "Tải xuống thất bại. Bạn có thể không có quyền truy cập.",
        );
      } else {
