...
} from "@/features/files/components/ConfirmUploadDialog";
...
export function FileExplorer({ folderId }: FileExplorerProps) {
  ...
  const handleRenameFileSubmit = useCallback(
    (newName: string) => {
      ...
      renameFileMutation.mutate(
        { fileId: renamingFile.id, newName },
        {
          onSuccess: () => {
  ...
  const processFiles = useCallback(
    async (inputFiles: FileList | File[] | PendingUploadFile[]) => {
      if (!inputFiles || inputFiles.length === 0 || !currentFolderId) return;

      let filesArray: PendingUploadFile[];
      if (
        inputFiles.length > 0 &&
        "path" in inputFiles[0] &&
        "file" in inputFiles[0]
      ) {
      ...
      if (pathsToResolve.size > 0) {
        toast.loading("Đang tạo cấu trúc thư mục...", {
          id: "resolve-folders",
        });
        try {
          for (const path of Array.from(pathsToResolve)) {
            const resolvedFolder = await resolveFolderPath({
              path,
          ...
          toast.error("Lỗi khi tạo cấu trúc thư mục", {
            id: "resolve-folders",
          });
          return;
        }
  ...
  if (isLoading) {
    return (
      ...
      >
        <Loader2
          className="w-8 h-8 animate-spin text-zinc-900 dark:text-zinc-100"
        ...
        <Box>
          {folderId && folderInfo && (
            <Flex align="center" gap="2" mb="2">
              ...
              >
                ...
                >
                  <Home className="w-4 h-4 mr-1" /> Tệp của tôi
                </Text>
              </Link>
          ...
          >
            {!folderId || !folderInfo ? (
              ...
              <FolderOpen
                style={{ width: 32, height: 32, color: "var(--amber-11)" }}
              />
            )}
      ...
      >
        ...
        {isDragging && (
          ...
          >
            ...
            >
              Chúng tôi sẽ liệt kê các tệp để bạn duyệt và xác nhận trước khi
              tải lên.
            </Text>
          </Flex>
        )}
        ...
        {folders.length === 0 && files.length === 0 ? (
          ...
          >
            ...
            >
              Kéo thả tệp vào đây hoặc nhấn mũi tên tải lên ở góc trên cùng để
              bắt đầu.
            </Text>
          </Flex>
        ) : (
                              ...
                              <DropdownMenu.Root>
                                ...
                                >
                                  ...
                                  <DropdownMenu.Item
                                    ...
                                    className="cursor-pointer"
                                  >
                                    <LinkIcon className="w-4 h-4 mr-2" /> Chia
                                    sẻ
                                  </DropdownMenu.Item>
                                  <DropdownMenu.Item
