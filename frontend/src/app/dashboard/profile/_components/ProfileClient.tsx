...
export function ProfileClient() {
  ...
  const errorMsg = updateProfileMutation.isError
    ? getApiErrorMessage(
  ...
  const storagePercent =
    maxStorage > 0 ? Math.round((usedStorage / maxStorage) * 100) : 0;
      ...
      <div className="relative rounded-3xl overflow-hidden mb-8 border border-border/50 bg-card">
        ...
        <div className="px-8 pb-8 relative flex flex-col sm:flex-row items-center sm:items-end gap-6 -mt-16 sm:-mt-20">
          <div className="relative p-2 bg-card rounded-full border border-border">
            ...
            <div
              className="absolute bottom-4 right-4 w-5 h-5 bg-emerald-500 border-4 border-card rounded-full"
          ...
          <div className="flex-1 text-center sm:text-left mb-2">
            ...
            {errorMsg && (
              <p className="text-rose-500 text-sm mt-2">{errorMsg}</p>
            )}
            {successMsg && (
              <p className="text-emerald-500 text-sm mt-2">{successMsg}</p>
            )}
          </div>
            ...
            {isEditing ? (
              <>
                <button
                  ...
                  className="px-4 py-2 bg-secondary text-foreground font-bold rounded-xl border border-transparent hover:border-gray-300 transition-colors text-sm whitespace-nowrap"
                >
                ...
                <button
                  ...
                  disabled={updateProfileMutation.isPending}
                  className="px-6 py-2 bg-primary text-white font-bold rounded-xl border border-transparent hover:border-primary/80 transition-colors text-sm flex items-center gap-2 whitespace-nowrap"
                >
              ...
              <>
                <button
                  ...
                  className="px-5 py-2.5 bg-background border border-border text-foreground font-bold rounded-xl hover:border-primary transition-colors text-sm flex items-center gap-2 whitespace-nowrap"
                >
                  <ShieldCheck className="w-4 h-4 inline text-primary" /> Đổi
                  mật khẩu
                </button>
                <button
                  onClick={handleStartEdit}
                  className="px-6 py-2.5 bg-secondary text-foreground font-bold rounded-xl border border-transparent hover:border-gray-200 transition-colors text-sm flex items-center gap-2 whitespace-nowrap"
                >
        ...
        <div className="space-y-8">
            ...
            <div className="relative w-40 h-40 mx-auto mb-6 flex justify-center items-center">
              <svg className="w-full h-full transform -rotate-90">
                ...
                <circle
                  ...
                  fill="transparent"
                  strokeDasharray="440"
                  strokeDashoffset={440 - (440 * storagePercent) / 100}
          ...
          <div className="bg-card border border-border/50 rounded-3xl p-6">
            ...
            <ul className="space-y-4 text-sm">
              ...
              <li className="flex items-center justify-between">
                <span className="text-muted-foreground flex items-center gap-2">
                  <Activity className="w-4 h-4" /> Đăng nhập lần cuối
                </span>
              ...
              <li className="flex items-center justify-between">
                <span className="text-muted-foreground flex items-center gap-2">
                  <Shield className="w-4 h-4" /> Xác thực 2 bước
                </span>
                {user.twoFactorEnabled ? (
                  <span className="font-semibold px-2 py-0.5 bg-emerald-500/10 text-emerald-500 rounded-lg border border-emerald-500/20 text-xs">
                    Đã bật
                  </span>
                ) : (
                  <span className="font-semibold px-2 py-0.5 bg-rose-500/10 text-rose-500 rounded-lg border border-rose-500/20 text-xs">
                    Chưa bật
                  </span>
                )}
              </li>
            ...
            <div className="relative pl-6 border-l-2 border-border/50 space-y-8">
              <div className="relative">
                ...
                <div className="bg-secondary/50 p-4 rounded-xl border border-border/50">
                  <p className="text-sm">
                    Bạn đã tải lên tệp{" "}
                    <span className="font-bold text-primary flex items-inline inline-flex px-1">
                      <FileText className="w-4 h-4 inline mr-1" />{" "}
                      Ban_ke_hoach_kinh_doanh_2024.pdf
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <div className="bg-secondary/50 p-4 rounded-xl border border-border/50">
                  <p className="text-sm">
                    ...
                    <span className="font-bold text-emerald-500 flex items-inline inline-flex px-1">
                      <Crown className="w-4 h-4 inline mr-1" /> FileFlow Pro
                    </span>
                  </p>
                </div>
              </div>
              ...
              <div className="relative">
                ...
                <div className="bg-secondary/50 p-4 rounded-xl border border-border/50">
                  <p className="text-sm">
                    Xóa 12 tệp cũ vào Thùng rác để giải phóng{" "}
                    <span className="font-mono font-bold">2.4 GB</span> dung
                    lượng tĩnh.
                  </p>
                </div>
              </div>
