...
import type { TrashResponse } from '../types/trash.types';
import type { StandardResponse } from '@/shared/api/api.types';

export const trashApi = {
  getTrashItems: async (): Promise<TrashResponse> => {
    const response = await axiosInstance.get<StandardResponse<TrashResponse>>('/api/trash');
